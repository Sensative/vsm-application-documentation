...
# Application Lifefinder-mandown-wifi
...
## Button Activated deviceActive trigger for lifefinder-timekeeper-geofence-motion.vsr


BACKUP register is read on startup, resuming active time in the event of a device crash or restart.


SHORT press toggles geo-fencing on/off. Active is indicated by one LED flash while inactive is indicated by two flashes.
LONG press manually triggers a motion alarm while geo-fencing is active.
LONG LONG press clears the motion alarm while geo-fencing stays active.


Motion alarm counts up in minutes while active. Device counts the minutes spent in active mode. If the device is active for longer than activeTimeMaxMinutes, it is set to inactive.
...
Current time spent in active mode is saved to the BACKUP register. If alarm is active, a negative value is stored
so both the active time and alarm state can be restored on startup after a device crash or restart.

## Motion tracking module for lifefinder-timekeeper-geofence.vsr
...
Motion alarm is triggered when the device has been stationary for 3 minutes while geo-fencing is active.
The device LED blinks continuously until the alarm is cleared by button press or movement.

## Positioning using WiFi for lifefinder-timekeeper-geofence.vsr
...
Device will position every stationaryPositionMinutes while stationary and no motion alarm is active.


Device will position every minute while motion alarm is active.
...
Maximum duration of one active geo-fencing session.


### Input alarmTimeMaximum (unconfirmed)
...
> - *Note: It is highly recommended to ensure that you use higher level applications to update settings so that the correct version of this application is used as reference (these data may change or differ between sensors)*
> - UI: Alarm Time Maximum
> - Unit: Minutes
> - Min: 0
> - Max: 32767
> - Default: 120

Maximum duration of one alarm state. Alarm is automatically cleared after this time.
...
### Input stationaryAlarmMinutes (unconfirmed)
...
> - *Note: It is highly recommended to ensure that you use higher level applications to update settings so that the correct version of this application is used as reference (these data may change or differ between sensors)*
> - UI: Stationary Alarm Minutes
> - Unit: Minutes
> - Min: 1
> - Max: 127
> - Default: 3

Amount of time in minutes without movement before a motion alarm is triggered.
