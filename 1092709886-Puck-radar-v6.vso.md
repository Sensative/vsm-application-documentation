...
Every hour, when the device is not joined it will attempt to join, for the first 24 hrs after NFC field is not detected.


At a 24hr interval the device will try to join should it not succeed withing the first 24 hours hourly attempts.

Every 24 hours when joined, the device will attempt a wifi scan.
Every 7 days when joined, the device will attempt a GNSS scan.
## Slow Tracker Module
