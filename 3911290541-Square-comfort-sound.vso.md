...
At an interval of averageHumidityIntervalMinutes the average is reported, changed or not.
humidityTreshold still gates the instantaneous humidity report above; it no longer gates the
average, because an interval report that goes missing is worse than one that repeats.
