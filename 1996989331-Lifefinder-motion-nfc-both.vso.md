...
# Application Lifefinder-motion-nfc-both
...
## Positioning using both WiFi and GNSS for lifefinder-timekeeper-nfc-motion.vsr
...
Device will check how many APs are found on the next minute and optionally try GNSS if less than minimumWifiResult.


Try a position every minute when the device is in alarm state


Try a GNSS position on the next minute if the WiFi scan did not find enough APs
...
### Input minimumWifiResult (unconfirmed)
...
> - *Note: It is highly recommended to ensure that you use higher level applications to update settings so that the correct version of this application is used as reference (these data may change or differ between sensors)*
> - UI: Minimum APs required
> - Unit: Integer
> - Min: 0
> - Max: 16
> - Default: 3

If less APs than this are found, the device will try to get a GNSS position instead a minute later.
